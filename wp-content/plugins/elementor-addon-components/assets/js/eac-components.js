	...
	var EacAddonsElements = {
		...
		*/
		widgetLecteurAudio: function($scope) {
			...
			$('.la-lecteur-audio', $target).mediaPlayer();
			...
			$(document).ajaxComplete(function(event, xhr, ajaxSettings) {
				if(ajaxSettings.ajaxOptions && ajaxSettings.ajaxOptions === instanceAjax.getOptions()) { // Le même random number généré lors de la création de l'objet Ajax
					...
					$.each(Items, function(index, item) {
						...
						var $wrapperItem = $('<div/>', { class :'rss-galerie__item ' + settings.data_style});
		...
		*/
		widgetSliderPro: function($scope) {
			var $target = $scope.find('.slider-pro'),
				...
				windowWidthTab = 768,	// Breakpoint pour les tablettes
				windowWidthMob = 640,	// Breakpoint pour les mobiles
				defaultSettings = {
					width: 600,
					height: 450,
					arrows: false,
					...
					init: function() {
						...
						if($(window).width() <= windowWidthMob) { defaultTexteAnimation = settingsAnime.data_anmpos_mob; }
						else if($(window).width() <= windowWidthTab) { defaultTexteAnimation = settingsAnime.data_anmpos_tab; }
		...
		*/
		widgetImageComparison: function($scope) {
			var $target = $scope.find('.images-comparison'),
				$targetInstance = $scope.find('.eac-images-comparison'),
				$imagesInstance = $target.find('img'),
				settings = $target.data('settings'),
				instance = null,
				targetInstanceWidth;
			...
			var imgCompWP = new Waypoint({
				element: $targetInstance,
				triggerOnce: true,
				offset: 99999,
				handler: function(direction) {
					$target.imagesLoaded().progress(function(instance, image) {
						if(image.isLoaded) {
							$(image.img).addClass('ic-image-loaded');
						}
					}).done(function() {
						// Toutes les images sont chargées, on instancie l'object
						$(settings.data_diff).simpleImageDiff();
					});
				}
			});
		},
		...
		widgetImageGalerie: function($scope) {
			...
			EacAddonsElements.widgetImageGalerie.collage = function() {
				$targetId.imagesLoaded().progress(function(instance, image) {
					...
					var justifyHeight = $(window).width() <= windowWidthMob ? settings.gridHeightM : $(window).width() <= windowWidthTab ? settings.gridHeightT : settings.gridHeight;
