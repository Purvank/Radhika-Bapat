...
if(!empty($_REQUEST['instagram']) && !empty($_REQUEST['cursor'])) {
	$endpoints->setAccountMediasCount(12); // Nombre de posts à retourner pour la pagination 36 = 12*3
	if(!setUserMedias($_REQUEST['instagram'], $_REQUEST['cursor'])) { exit; }
	if(!empty($items['mediasHashtags'])) { unset($items['mediasHashtags']); }
} else {
	$endpoints->setAccountMediasCount(25); // Nombre de posts à retourner pour la pagination 75 = 25*3
