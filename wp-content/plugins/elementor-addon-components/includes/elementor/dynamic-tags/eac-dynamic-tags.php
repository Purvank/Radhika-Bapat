...
*           'get_elementor_templates' Liste de tous les templates Elementor
...
* @since 1.7.5	Ajout des Dynamic Tags ACF
*========================================================================================*/
...
class Eac_Dynamic_Tags {
	...
	public function __construct() {
		add_action('elementor/dynamic_tags/register_tags', array($this, 'eac_register_tags'));
	}
	...
	* @since 1.6.0
	*/
	public function eac_register_tags($dynamic_tags) {
		...
		if(class_exists('\acf') && function_exists('acf_get_field_groups')) {
			// Charge le module ACF et les méthode afférentes pour les tags
			require_once(__DIR__ . '/acf/eac-acf-tags.php');
			
			require_once(__DIR__ . '/acf/tags/acf-field-keys.php');
			...
			require_once(__DIR__ . '/acf/tags/acf-field-values.php');
		...
		if(!empty($data) && !is_wp_error($data)) {
			foreach($data as $key) {
				//if(!function_exists('pll_the_languages')) {
				    $post_list[get_permalink($key->ID)] = $key->post_title;
				/*} else { // PolyLang
				    $post_id_lang = pll_get_post($key->ID);
		            if($post_id_lang) {
			            $post_list[get_permalink($post_id_lang)] = $key->post_title;
		            }
				}*/
			}
		}
