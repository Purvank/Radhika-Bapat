...
class Modal_Box_Widget extends Widget_Base {
	...
	*/
	protected function _register_controls() {
		...
		$this->start_controls_section('mb_param_trigger',
			...
			$this->add_responsive_control('mb_image_dimension',
				[
			...
			$this->add_responsive_control('mb_align_image',
				[
		...
		$this->start_controls_section('mb_header_style',
			...
			$this->add_responsive_control('mb_header_padding',
				[
		...
		$this->start_controls_section('mb_image_style',
			...
			$this->start_controls_tabs('mb_image_style_normal');
				...
				$this->start_controls_tab('mb_image_tab_style_normal',
					...
					$this->add_responsive_control('mb_image_padding',
						[
							'label'         => __('Marges internes', 'eac-components'),
							'type'          => Controls_Manager::DIMENSIONS,
							'size_units'    => ['px', 'em'],
		...
		$this->start_controls_section('mb_legende_style',
			...
			$this->add_responsive_control('mb_legende_margin',
				[
	...
	*/
	...
	protected function render_galerie() {
		...
		// Unique ID du widget. Fourni par Elementor
		$id = $this->get_id();
		...
		// Le déclencheur est une image
			...
			$this->add_render_attribute('trigger', 'src', $settings['mb_display_image']['url']);
		...
		<div <?php echo $this->get_render_attribute_string('mb_wrapper') ?>>
			...
			if($has_caption) {
				if('attachment' === $settings['mb_caption_source'] && !empty($settings['mb_display_image']['id'])) {
					...
					$caption = !empty($settings['mb_caption_texte']) ? $settings['mb_caption_texte'] : '';
				}
			}
			...
			<?php endif; ?>
					<?php if('button' === $trigger) :							// déclencheur bouton ?>
					...
					<?php elseif('image' === $trigger) :						// déclencheur image ?>
						...
						<img <?php echo $this->get_render_attribute_string('trigger'); ?>>
							...
							<?php endif; ?>
						<?php if(!empty($caption)) : ?></figure><?php endif; ?>
					<?php elseif('text' === $trigger) :							// déclencheur texte ?>
