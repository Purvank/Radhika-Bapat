...
class Image_Promotion_Widget extends Widget_Base {
    ...
    protected function _register_controls() {
		...
		$this->start_controls_section('ip_image_settings',
			...
			$this->add_control('ip_icon_switcher',
				[
					'label' => __("Ajouter une icône", 'eac-components'),
					'type' => Controls_Manager::SWITCHER,
					'label_on' => __('oui', 'eac-components'),
					'label_off' => __('non', 'eac-components'),
					'return_value' => 'yes',
					'default' => '',
					'condition' => ['ip_image_switcher' => ''],
				]
			);
			...
			$this->add_responsive_control('ip_image_align',
				[
		...
		$this->start_controls_section('ip_global_settings',
			...
			$this->add_control('ip_ribbon_switcher',
				[
					'label' => __("Ruban", 'eac-components'),
					'description'	=> __("Ajouter un ruban", 'eac-components'),
					'type' => Controls_Manager::SWITCHER,
					'label_on' => __('oui', 'eac-components'),
					'label_off' => __('non', 'eac-components'),
					'return_value' => 'yes',
		...
		$this->start_controls_section('ip_image_style',
           [
               'label' => __('Image', 'eac-components'),
               'tab' => Controls_Manager::TAB_STYLE,
			   'condition' => ['ip_image_switcher' => 'yes'],
		...
		$this->start_controls_section('ip_icone_style',
           [
               'label' => __('Icône', 'eac-components'),
               'tab' => Controls_Manager::TAB_STYLE,
			   'condition' => ['ip_icon_switcher' => 'yes'],
           ]
		);
			...
			$this->add_control('ip_icone_forme',
				[
					...
					'default' => 'circle',
					'condition' => [
						'ip_icone_voir!' => 'default',
					],
					'prefix_class' => 'elementor-shape-',
				]
			);
			
			$this->add_responsive_control('ip_icone_align',
				[
					'label' => __('Alignement', 'eac-components'),
					'type' => Controls_Manager::CHOOSE,
					'options' => [
			...
			$this->add_responsive_control('ip_icone_size',
				[
					...
					'desktop_default' => ['size' => 60, 'unit' => 'px'],
		...
		$this->start_controls_section('ip_carac_style',
           [
			...
			$this->add_responsive_control('ip_carac_align',
				[
					'label' => __('Alignement', 'eac-components'),
					'type' => Controls_Manager::CHOOSE,
					'options' => [
    ...
    */
    ...
    protected function render_galerie() {
		...
		if(! empty($settings['ip_icon_content'])) {
			$link_icon = true;
			$this->add_render_attribute('icon', 'class', $settings['ip_icon_content']); 
		...
		if($settings['ip_link_to'] === 'custom') {
			$link_url = $settings['ip_link_url']['url'];
            ...
            $this->add_render_attribute('ip-link-to', 'href', get_permalink(get_page_by_title($link_url)));
		}
		...
		<?php if(!empty($image_url)) : ?>
			<figure class="ip-image">
				<?php if($visionneuse) : ?>
					<a <?php echo $this->get_render_attribute_string('ip-lightbox'); ?>>
				<?php endif; ?>
				<?php echo Group_Control_Image_Size::get_attachment_image_html($settings, '', 'ip_image_content'); ?>
				<?php if($visionneuse) : ?>
					</a>
				<?php endif; ?>
			</figure>
		...
		<div <?php echo $this->get_render_attribute_string('wrapper-button'); ?>>
			...
			<?php endif; ?>
				<span <?php echo $this->get_render_attribute_string('button'); ?>>
					<i class="fa fa-arrow-right" aria-hidden="true"></i><?php echo $settings['ip_button_text']; ?>
				</span>
			<?php if(! empty($link_url)) : ?>
				</a>
			<?php endif; ?>
