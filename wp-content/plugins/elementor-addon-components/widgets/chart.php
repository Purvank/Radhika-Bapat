...
class Chart_Widget extends Widget_Base {
	...
	*/
	...
	?>
		<div class="eac-chart">
			<?php $this->render_galerie($json_data); ?>
		</div>
	<?php
    }
	
	protected function render_galerie($datajson) {
		$settings = $this->get_settings_for_display();
	...
	*/
	protected function get_settings_json($rid, $sid, $did, $data_json=[]) {
		...
		if(!empty($data_json)) {
			foreach($data_json['datasets'] as $item) {
				...
				if($module_settings['chart_type'] === 'bar' && isset($item['type']) && $item['type'] === 'line') {
				    ...
				    $samescale = 1;
				    $y2title = isset($data_json['options']['y_axis2']['title']) ? $data_json['options']['y_axis2']['title'] : ''; // Le titre
				    $y2label = isset($item['label']) ? $item['label'] : '';
				    $y2data = isset($item['data']) ? $item['data'] : '';
				} else {
                    array_push($arrayLabel, $item['label']);
			...
			foreach($module_settings['chart_y_data_list'] as $item) {
				if(! empty($item['chart_y_data'])) {
					array_push($arrayLabel, $item['chart_y_legend']);
		...
		$settings = array(
			...
			"x_title"	    => !empty($data_json) && isset($data_json['options']['x_axis']['title']) ? $data_json['options']['x_axis']['title'] : $module_settings['chart_x_title'],
			
			// Plusieurs séries de données. Séparateur = point-virgule pour chaque série de données
			"y_data"        => implode(";", $arrayDataSeries),
			"y_title"	    => !empty($data_json) && isset($data_json['options']['y_axis']['title']) ? $data_json['options']['y_axis']['title'] : $module_settings['chart_y_title'],
