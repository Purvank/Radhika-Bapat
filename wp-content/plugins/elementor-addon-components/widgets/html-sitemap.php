...
class Html_Sitemap_Widget extends Widget_Base {
	...
	*/
    protected function render() {
		...
		$display_class = vsprintf('desktop-column-%s tablet-column-%s mobile-column-%s', $this->init_settings());
		// Pas d'article
		//if(empty($settings['stm_article_type'])) {
			//return;
		//}
		?>
	...
	protected function eac_get_html_sitemap_authors() {
		$settings = $this->get_settings_for_display();
		$class = vsprintf("sitemap-authors desktop-column-%s tablet-column-%s mobile-column-%s", $this->init_settings());
		$this->add_render_attribute('sitemap_wrapper_author', 'class', $class);
		?>
		<div <?php echo $this->get_render_attribute_string('sitemap_wrapper_author'); ?>>
			<div class="sitemap-posts-title"><h2><?php echo $settings['stm_author_titre']; ?></h2></div>
	...
	protected function eac_get_html_sitemap_pages() {
		$settings = $this->get_settings_for_display();
		$class = vsprintf("sitemap-pages desktop-column-%s tablet-column-%s mobile-column-%s", $this->init_settings());
		$this->add_render_attribute('sitemap_wrapper_page', 'class', $class);
		?>
		<div <?php echo $this->get_render_attribute_string('sitemap_wrapper_page'); ?>>
			<div class="sitemap-posts-title"><h2><?php echo $settings['stm_page_titre']; ?></h2></div>
			<div class="sitemap-posts-list">
				<?php
	...
	protected function eac_get_html_sitemap_archives() {
		$settings = $this->get_settings_for_display();
		$class = vsprintf("sitemap-archives desktop-column-%s tablet-column-%s mobile-column-%s", $this->init_settings());
		$this->add_render_attribute('sitemap_wrapper_archive', 'class', $class);
		?>
		<div <?php echo $this->get_render_attribute_string('sitemap_wrapper_archive'); ?>>
			<div class="sitemap-posts-title"><h2><?php echo $settings['stm_archive_titre']; ?></h2></div>
			<div class="sitemap-posts-list">
				<?php
	...
	protected function eac_get_html_sitemap_taxonomies() {
		$settings = $this->get_settings_for_display();
		$class = vsprintf("sitemap-taxonomies desktop-column-%s tablet-column-%s mobile-column-%s", $this->init_settings());
		$this->add_render_attribute('sitemap_wrapper_taxo', 'class', $class);
		?>
		<div <?php echo $this->get_render_attribute_string('sitemap_wrapper_taxo'); ?>>
			<div class="sitemap-posts-title"><h2><?php echo $settings['stm_taxonomy_titre']; ?></h2></div>
			<div class="sitemap-posts-list">
				<ul>
				<?php
				global $post;
							...
							if(count($posts_array) > 0) {
								...
								<li><span><span><b><?php echo ucfirst($taxo); ?></b></span><a href="<?php echo esc_url(get_category_link($categorie->cat_ID)); ?>" <?php echo $has_nofollow; ?>><?php echo $categorie->cat_name; ?></a><?php echo $cat_count; ?></span>
									...
									foreach($posts_array as $post) {
										...
										?>
										<li><span><?php echo $la_date; ?><a href="<?php esc_url(the_permalink()); ?>" title="<?php the_title(); ?>" <?php echo $has_nofollow; ?>><?php the_title(); ?></a><?php echo $c_count;?><?php echo $id; ?></span></li>
										<?php 
									}
									wp_reset_postdata();
									?>
	...
	protected function eac_get_html_sitemap_posts() {
		$settings = $this->get_settings_for_display();
		$class = vsprintf("sitemap-posts desktop-column-%s tablet-column-%s mobile-column-%s", $this->init_settings());
		$this->add_render_attribute('sitemap_wrapper_post', 'class', $class);
		?>
		<div <?php echo $this->get_render_attribute_string('sitemap_wrapper_post'); ?>>
			<div class="sitemap-posts-title"><h2><?php echo $settings['stm_post_titre']; ?></h2></div>
			<div class="sitemap-posts-list">
				<?php
				global $post;
				...
				foreach($post_types as $post_type) { 
					if(empty($exclude_posttype) || (!empty($exclude_posttype) && !in_array($post_type->name, $exclude_posttype))) { ?>
						...
						foreach($posts_array as $post) {
							...
							?>
							<li><span><?php echo $categories . $la_date; ?><a href="<?php esc_url(the_permalink()); ?>" <?php echo $has_nofollow; ?>><?php the_title(); ?></a><?php echo $c_count;?><?php echo $id; ?></span></li>
						<?php
						}
						wp_reset_postdata();
						?>
	...
	protected function init_settings() {
		...
		$columns = $module_settings['stm_setting_column'] == '' ? '3' : $module_settings['stm_setting_column'];
		$columns_tab = $module_settings['stm_setting_column_tablet'] == '' ? '2' : $module_settings['stm_setting_column_tablet'];
		$columns_mob = $module_settings['stm_setting_column_mobile'] == '' ? '1' : $module_settings['stm_setting_column_mobile'];
