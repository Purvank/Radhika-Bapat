...
class Slider_Pro_Widget extends Widget_Base {
    ...
    protected function _register_controls() {
		...
		$this->start_controls_section('spro_slider_settings',
			...
			$this->add_responsive_control('spro_slider_width',
				[
					'label' => __("Largeur du slider (px)", 'eac-components'),
					'type'  => Controls_Manager::SLIDER,
					'size_units' => ['px'],
					'devices' => ['desktop', 'tablet', 'mobile'],
					'desktop_default' => ['size' => 600, 'unit' => 'px'],
			...
			$this->add_responsive_control('spro_slider_height',
				[
					'label' => __("Hauteur du slider (px)", 'eac-components'),
					'type'  => Controls_Manager::SLIDER,
					'size_units' => ['px'],
					'devices' => ['desktop', 'tablet', 'mobile'],
					'desktop_default' => ['size' => 450, 'unit' => 'px'],
					'tablet_default' => ['size' => 350, 'unit' => 'px'],
		...
		$this->start_controls_section('spro_titre_style',
			...
			$this->add_control('spro_titre_color',
				[
					...
					'default' => '#FFF',
					'selectors' => [
						'{{WRAPPER}} .spro-slide-title .spro-slide-title-texte' => 'color: {{VALUE}};',
						//'{{WRAPPER}} .spro-slide-title .spro-slide-title-texte:hover' => 'color: #bab305;',
					],
				]
			);
    ...
    protected function render_galerie() {
		...
		foreach($settings['spro_image_list'] as $item) {
		    if(!empty($item['spro_item_image']['url'])) { // Il y a une image
		        $link_url = $item['spro_item_url']['url'];
				/**
				...
				if(!empty($item['spro_item_image']['id'])) {
				    ...
				    $url = $item['spro_item_image']['url'];
				    // @since 1.4.1 Ajout du paramètre 'ver' à l'image avec un identifiant unique pour forcer le chargement de l'image du serveur et non du cache
				    $image = sprintf('<img class="eac-image-loaded sp-image" src="%s?ver=%s" alt="%s" />', $url, uniqid(), $image_alt);
				}
				
				$titre = !empty($item['spro_item_title']) ? $item['spro_item_title'] : __('Pas de titre', 'eac-components');
				$caption = !empty($item['spro_item_desc']) ? $item['spro_item_desc'] : $image_alt;
	...
	*/
	protected function get_settings_json() {
		...
		$sliderwidth = !empty($module_settings['spro_slider_width']['size']) ? $module_settings['spro_slider_width']['size'] : 600;
		$sliderheight = !empty($module_settings['spro_slider_height']['size']) ? $module_settings['spro_slider_height']['size'] : 450;
		...
		$thumbswidth = !empty($module_settings['spro_control_thumbs_width']['size']) ? $module_settings['spro_control_thumbs_width']['size'] : 120;
		$thumbsheight = !empty($module_settings['spro_control_thumbs_height']['size']) ? $module_settings['spro_control_thumbs_height']['size'] : 80;
	...
	*/
	protected function get_settingsanime_json($dataid) {
		...
		$settings = array(
			...
			"data_anmpos" => $module_settings['spro_slider_animation'],
			"data_anmpos_tab" => $module_settings['spro_slider_animation_tablet'],
			"data_anmpos_mob" => $module_settings['spro_slider_animation_mobile'],
			"data_width_tab" => !empty($module_settings['spro_slider_width_tablet']['size']) ? $module_settings['spro_slider_width_tablet']['size'] : 500,
			"data_width_mob" => !empty($module_settings['spro_slider_width_mobile']['size']) ? $module_settings['spro_slider_width_mobile']['size'] : 400,
			"data_height_tab" => !empty($module_settings['spro_slider_height_tablet']['size']) ? $module_settings['spro_slider_height_tablet']['size'] : 350,
			"data_height_mob" => !empty($module_settings['spro_slider_height_mobile']['size']) ? $module_settings['spro_slider_height_mobile']['size'] : 250,
		);
