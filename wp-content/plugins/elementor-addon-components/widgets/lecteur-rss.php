...
class Lecteur_Rss_Widget extends Widget_Base {
    ...
    */
    protected function render() {
		...
		$class = vsprintf('rss-galerie desktop-column-%s tablet-column-%s mobile-column-%s', $this->init_settings());
		$this->add_render_attribute('rss_galerie', 'class', $class);
				...
				<select id="rss__options-items" class="rss__options-items">
					<?php foreach($settings['rss_image_list'] as $item) { ?>
						<?php if(! empty($item['rss_item_url']['url'])) : ?>
							<option value="<?php echo $item['rss_item_url']['url']; ?>"><?php echo $item['rss_item_title']; ?></option>
						<?php endif; ?>
					<?php } ?>
				</select>
			</div>
	...
	protected function init_settings() {
		...
		$columns = $module_settings['rss_columns'] == '' ? '3' : $module_settings['rss_columns'];
		$columns_tab = $module_settings['rss_columns_tablet'] == '' ? '2' : $module_settings['rss_columns_tablet'];
		$columns_mob = $module_settings['rss_columns_mobile'] == '' ? '1' : $module_settings['rss_columns_mobile'];
