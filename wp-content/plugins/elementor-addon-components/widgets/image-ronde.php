	...
	*/
    ...
    protected function _register_controls() {
		...
		$this->start_controls_section('ir_image_settings',
			...
			$this->add_responsive_control('ir_image_align',
				[
		...
		$this->start_controls_section('ir_links_settings',
			...
			$this->add_control('ir_icon_switcher',
				[
					'label' => __("Icône du lien", 'eac-components'),
					'type' => Controls_Manager::SWITCHER,
					'label_on' => __('oui', 'eac-components'),
					'label_off' => __('non', 'eac-components'),
					'return_value' => 'yes',
			...
			$this->add_control('ir_icon_for_url',
				[
			...
			[
			   ...
			   'conditions' => [
					...
					'terms' => [
						...
						//['name' => 'ir_icon_switcher', 'operator' => 'in', 'value' => ['yes']],
    ...
    */
    ...
    protected function render_galerie() {
		...
		if(! empty($settings['ir_image_content']['url'])) {
			$image_url = $settings['ir_image_content']['url'];
		...
		if($settings['ir_link_to'] == 'custom') {
			$link_url = $settings['ir_link_url']['url'];
            ...
            $this->add_render_attribute('ir-link-to', 'href', get_permalink(get_page_by_title($link_url)));
		}
		
		// Une icône de lien
		if('yes' === $icon_switcher) {
			if(! empty($settings['ir_icon_for_url'])) {
				$link_icon = true;
				$this->add_render_attribute('icon', 'class', $settings['ir_icon_for_url']);
		...
		if('yes' === $link_switcher && ! empty($link_url)) { $html .= '<a '. $this->get_render_attribute_string('ir-link-to') .'>';	}
			$html .= '<span class="titre-wrapper">' . $open_title . $settings['ir_title'] . $close_title . '</span>';
