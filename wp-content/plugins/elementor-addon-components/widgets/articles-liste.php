...
class Articles_Liste_Widget extends Widget_Base {
	...
	public function __construct($data = [], $args = null) {
		...
		// Filtre la liste des types d'articles
		/*add_filter('eac/tools/post_types', function($filter_posttypes) {
			$exclude = ['custom_css', 'customize_changeset', 'oembed_cache', 'user_request', 'wp_block', 'custom-css-js', 'flamingo_contact', 'flamingo_inbound', 'flamingo_outbound', 'wpforms', 'wpforms_log', 'wpcf7_contact_form', 'forminator_forms', 'forminator_polls', 'forminator_quizzes'];
			//return array_merge($filter_posttypes, $exclude);
			
			$include = ['custom_css'];
			return array_diff($filter_posttypes, $include);
		}, 10);*/
	...
	*/
	...
	public function get_keywords() {
		return ['post', 'custom post_type', 'query', 'filter', 'advanced', 'category', 'authors', 'poat_tag', 'custom fields'];
	}
    ...
    */
	protected function _register_controls() {
		...
		$this->start_controls_section('al_article_content',
			...
			$this->add_responsive_control('al_image_dimension',
				[
					'label'   => __('Dimension', 'eac-components'),
					'type'    => Controls_Manager::SELECT,
					'devices' => ['desktop', 'tablet', 'mobile'],
					'desktop_default' => 'medium',
					...
					/*'options' => Eac_Tools_Util::get_thumbnail_sizes(),*/
					'condition' => ['al_content_image' => 'yes'],
					/*'prefix_class' => 'al-image-dimension-%s',*/
				]
			);
		...
		$this->start_controls_section('al_general_style',
			...
			$this->add_control('al_wrapper_margin',
				[
		...
		$this->start_controls_section('al_image_style',
			...
			/** @since 1.7.0 Le ratio appliqué à l'image */
			...
			/** @since 1.7.2 Positionnement vertical */
			$this->add_responsive_control('al_image_ratio_position_y',
				[
					...
					'selectors' => ['{{WRAPPER}} .al-posts__wrapper.al-posts__image-ratio .al-image-loaded' => 'object-position: 50% {{SIZE}}{{UNIT}};'],
					'condition' => ['al_enable_image_ratio' => 'yes', 'al_layout_type' => 'fitRows'],
				]
			);
	...
	* <img class="eac-image-loaded" src="<?php echo esc_url($image[0]);?>?nocache=<?php echo uniqid('', true); ?>" alt="<?php echo esc_html(get_the_title()); ?>" />
	* -------------------------------------
	* <?php the_post_thumbnail($settings['al_image_dimension'], array('alt' => esc_html(get_the_title()), 'class' => 'eac-image-loaded')); ?>
	*
	*-------------------------------------*/
	...
	protected function render_galerie() {
		...
		// La div wrapper
		$class = vsprintf("al-posts__wrapper %s layout-type-%s desktop-column-%s tablet-column-%s mobile-column-%s", $this->init_settings());
			...
			<div <?php echo $this->get_render_attribute_string('posts_wrapper'); ?>>
				...
				?>	
					<article id="<?php echo 'post-' . get_the_id(); ?>" class="<?php echo $unique_id; ?> al-post__wrapper <?php echo implode(' ', $termsSlug); ?>">
						<div class="al-post__wrapper-inner <?php echo $settings['al_wrapper_style']; ?>">
						    <?php if($has_titre) : ?>
        						<!-- Le titre -->
        						<?php if($has_titre) : ?>
        							<!-- Affiche les IDs -->
							...
							<div <?php echo $this->get_render_attribute_string('content_wrapper'); ?>>
								...
								<div class="al-text__content-wrapper">
									...
									<!-- Le lien pour ouvrir l'article/page -->
							...
							<div class="al-meta__content-wrapper">
								...
								<div class="al-post__meta">
									...
									<!-- Le date de dernière modification -->
									<?php if($has_date) : ?>
	...
	protected function init_settings() {
		...
		$columns = $module_settings['al_columns'] == '' ? '3' : $module_settings['al_columns'];
		$columns_tab = $module_settings['al_columns_tablet'] == '' ? '2' : $module_settings['al_columns_tablet'];
		$columns_mob = $module_settings['al_columns_mobile'] == '' ? '1' : $module_settings['al_columns_mobile'];
		$ratio = $module_settings['al_enable_image_ratio'] === 'yes' ? ' al-posts__image-ratio' : '';
		
		return array($ratio, $layout, $columns, $columns_tab, $columns_mob);
	}
