...
*
* Description: Pinterest_pin_Widget affiche la liste des flux
...
class Pinterest_Rss_Widget extends Widget_Base {
    ...
    */
    protected function render() {
		...
		$class = vsprintf('pin-galerie desktop-column-%s tablet-column-%s mobile-column-%s', $this->init_settings());
		$this->add_render_attribute('pin_galerie', 'class', $class);
				...
				<select id="pin_options_items" class="pin-options-items">
					<?php foreach($settings['pin_pinterest_list'] as $item) { ?>
						...
						<?php if(! empty($item['pin_item_url']['url']) && ! empty($item['pin_item_user'])) : ?>
							<?php if($has_board) : ?>
								<?php $url = $item['pin_item_url']['url'] . '/' . $item['pin_item_user'] . '/' . $item['pin_item_board'] . $board; ?>
							<?php else : ?>
								<?php $url = $item['pin_item_url']['url'] . '/' . $item['pin_item_user'] . $user; ?>
							<?php endif; ?>
							<option value="<?php echo $url; ?>"><?php echo $item['pin_item_title']; ?></option>
						<?php endif; ?>
					<?php } ?>
				</select>
			</div>
	...
	protected function init_settings() {
		...
		$columns = $module_settings['pin_columns'] == '' ? '3' : $module_settings['pin_columns'];
		$columns_tab = $module_settings['pin_columns_tablet'] == '' ? '2' : $module_settings['pin_columns_tablet'];
		$columns_mob = $module_settings['pin_columns_mobile'] == '' ? '1' : $module_settings['pin_columns_mobile'];
