...
class Feed {
	...
	public function display_podcast( $args ) {
		 ...
		 * @param array  $args Podcast display args.
		 */
		$mod_data = apply_filters( 'podcast_player_feed_mods', $mod_data, $args );

		// Let's add fixed item in args to be send to front-end script.
		$args['fixed'] = isset( $mod_data['fixed'] ) ? $mod_data['fixed'] : '';
		...
		if ( is_wp_error( $feed ) ) {
			if ( is_admin() || current_user_can( 'manage_options' ) ) {
				...
				echo apply_filters( 'podcast_player_feed_error_msg', '', $feed, $args );
			}
			return;
		}
		...
		list( $imgurl, $oricov, $imgset ) = $this->get_cover_image( $args, $feed );
		$args['imgurl']                   = $imgurl;
		$args['imgset']                   = $imgset;
		$args['oricov']                   = $oricov;
		...
		$this->add_podcast_script_data( $fitems, $floaded, $totalitems, $args['url'], $args, $number, $title );
	...
	private function get_required_data_fields( $args ) {
		$default_fields = array( 'title', 'date', 'link', 'src', 'mediatype' );
		$conditional    = array(
	...
	private function get_cover_image( $args, $feed ) {
		...
		return array( $url, $oricov, $srcset );
	}
	...
	private function prepare_feed_items( $items, $counter, $items_count, $args = array() ) {
		...
		foreach ( $items as $key => $item ) {
			...
			if ( isset( $item['featured'] ) ) {
				list( $featured, $srcset ) = $this->get_item_featured( $item, $args );
	 ...
	 */
	private function add_podcast_script_data( $script_data = array(), $loaded, $total, $url, $args, $counter, $title ) {
		global $wp;
