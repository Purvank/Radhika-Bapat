...
class Render {
	...
	public function __construct( $props ) {
		// Extract all props.
		list( $title, $desc, $link, $items, $instance, $args ) = $props;
	...
	private function get_wrapper_classes() {
		...
		if ( Validation_Fn::is_single_episode_layout( count( $this->items ), $style ) ) {
			$cls[] = 'single-episode';
	...
	private function get_dynamic_css() {
		...
		if ( $this->args['accent-color'] ) {
			...
			$css  .= sprintf(
				...
				%4$s.playerview .pod-entry.activeEpisode {
	...
	private function display_podcast_player( $classes, $styles ) {
		...
		$markup     = sprintf(
			'<div id="pp-podcast-%1$s" class="%2$s" %3$s>%4$s</div>',
	...
	private function pod_launch() {
		...
		return $this->markup(
			'pod-launch',
			array(
				'pod-launcher__menu'    => 'launcher_menu',
				'pod-launcher__default' => 'launch_btn',
			)
		);
	}
	...
	public function podcast_desc( $classname ) {
		...
		return sprintf( '<div class="%s">%s</div>', esc_attr( $classname ), esc_html( $this->info['description'] ) );
	}
	...
	public function podcast_menu() {
		...
		$has_subscribe = (bool) $menu_markup;
		$menu_markup  .= $this->get_podcast_menu( $has_subscribe );

		if ( $has_subscribe ) {
			$menu_markup = sprintf( '<div class="pod-items__navi-menu">%s</div>', $menu_markup );
		}
	...
	public function get_podcast_menu( $has_sub ) {
		...
		} elseif ( 'feed' === $this->args['fetch-method'] ) {
			$nav_menu = $this->get_default_feed_links();
		}
	...
	public function launch_btn() {
		...
		$display_btn = $is_cover || $is_title || $is_desc;
		if ( $display_btn ) {
	...
	public function search_field( $classname ) {
		...
		// OR Return if there is only one episode in the list.
		if ( $this->args['hide-search'] || 1 >= count( $this->items ) ) {
			return '';
		}
	...
	public function is_header_visible() {
		...
		if ( ( '' === $this->args['display-style'] || 'legacy' === $this->args['display-style'] ) && ! $this->args['header-default'] ) {
			return false;
		} else {
			return true;
		}
